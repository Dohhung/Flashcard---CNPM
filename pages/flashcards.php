...
$query = "
    SELECT vs.*, u.role
    FROM vocabulary_set vs
    LEFT JOIN user u ON vs.user_id = u.user_id
    WHERE vs.vocabularySet_id = ? 
      AND (vs.user_id = ? OR u.role = 'admin')
";
...
<body>
    ...
    <div class="container mt-5">
        ...
        <div class="text-first mb-3">
            ...
            <a href="add_flashcard.php?vocabularySet_id=<?php echo $vocabularySet_id; ?>" class="btn btn-primary">Thêm Từ Vựng</a>
            <?php endif; ?>
            <?php if (!empty($flashcards_data)): ?>
                <div class="card mx-auto shadow-sm" style="max-width: 400px;">
                    <?php if (!empty($current_flashcard['image_path'])): ?>
                        <img src="<?php echo htmlspecialchars($current_flashcard['image_path']); ?>" class="card-img-top" alt="<?php echo htmlspecialchars($current_flashcard['vocab']); ?>">
                    <?php endif; ?>
                    <div class="card-body text-center">
                        ...
                        <div class="dropdown text-end mt-3">
                            ...
                            <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdownMenuButton<?php echo $current_flashcard['flashcard_id']; ?>">
                                <!-- Sửa Flashcard -->
                                <li>
                                    <a class="dropdown-item btn btn-warning" href="edit_flashcard.php?flashcard_id=<?php echo $vocabularySet_id; ?>&flashcard_id=<?php echo $current_flashcard['flashcard_id']; ?>">Sửa</a>
                                </li>
                                <!-- Xóa Flashcard -->
                                <li>
                                <a class="dropdown-item btn btn-warning" href="delete_flashcard.php?vocabularySet_id=<?php echo $vocabularySet_id; ?>&flashcard_id=<?php echo $current_flashcard['flashcard_id']; ?>" class="btn btn-danger" onclick="return confirm('Bạn có chắc chắn muốn xóa flashcard này?')">Xóa</a>
                                </li>
                            </ul>
                        </div>
                    <?php endif; ?>
            ...
            <?php else: ?>
                <div class="card-body text-center">
                    <h5 class="card-title text-danger">Chưa có flashcard nào trong bộ từ vựng này.</h5>
                </div>
            <?php endif; ?>
            <div class="d-flex justify-content-between mt-4">
                <a href="flashcards.php?vocabularySet_id=<?php echo $vocabularySet_id; ?>&index=<?php echo max(0, $current_index - 1); ?>" class="btn btn-secondary">Trước</a>
                <a href="flashcards.php?vocabularySet_id=<?php echo $vocabularySet_id; ?>&index=<?php echo min(count($flashcards_data) - 1, $current_index + 1); ?>" class="btn btn-primary">Tiếp theo</a>
            </div>
        </div>
    </div>

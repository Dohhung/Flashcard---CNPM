...
$user_query = "
    SELECT 
        ...
        u.phoneNumber, 
        u.dateSignin,
        MAX(h.activity_date) AS time_login,
        MAX(ADDTIME(h.activity_date, h.duration)) AS time_logout
    FROM 
        user u
    LEFT JOIN 
        history h ON u.user_id = h.user_id
    WHERE 
        u.role != 'admin' $search_query
    GROUP BY 
        u.user_id
    LIMIT ?, ?
";
                ...
                <?php if ($users->num_rows > 0): ?>
                    <?php while ($user = $users->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td><?= htmlspecialchars($user['time_login'] ?? 'Chưa có'); ?></td>
                            <td><?= htmlspecialchars($user['time_logout'] ?? 'Chưa có'); ?></td>
                            <td>
